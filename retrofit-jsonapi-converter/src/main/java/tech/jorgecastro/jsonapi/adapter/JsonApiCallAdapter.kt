...
class JsonApiRetrofiCall<T>(val call: Call<T>): Call<T> {
...
): CallAdapter<T, JsonApiRetrofiCall<T>> {
    override fun adapt(call: Call<T>): JsonApiRetrofiCall<T> {
        return JsonApiRetrofiCall(call)
    }
